...
class mainMenuUI:
    ...
    def show(self):
        ...
        mainMenu.blit(levelSelect_text,
                      (self.width // 2 - levelSelect_text.get_width() // 2, 2 * self.height // 3 + 70))
    ...
    def getStartDimensions(self):
        start_text = buttonFont.render("Start", 0, (0, 0, 0, 255))
        return Rect(self.width // 2 - start_text.get_width() // 2 - 5, 2 * self.height // 3 - 5,
                    start_text.get_width() + 10, start_text.get_height() + 10)

    def getLevelSelectDimensions(self):
        levelSelect_text = buttonFont.render("Level Select", 0, (0, 0, 0, 255))
        return Rect(self.width // 2 - levelSelect_text.get_width() // 2, 2 * self.height // 3 + 70,
                    levelSelect_text.get_width() + 10, levelSelect_text.get_height() + 10)
    ...
    def show(self):
        ...
        for i in range(4):
            for j in range(3):
                ...
                if Rect(80 + 300 * i, 220 + 130 * j, level_text.get_width(), level_text.get_height()).collidepoint(
                        self.mx, self.my):
